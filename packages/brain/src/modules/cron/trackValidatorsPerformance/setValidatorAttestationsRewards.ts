...
export async function setValidatorAttestationsRewards({
  ...
  const { total_rewards, ideal_rewards } = (
    await beaconchainApi.getAttestationsRewards({
      epoch,
      pubkeysOrIndexes: Array.from(validatorsDataPerEpochMap.keys()).map((index) => index.toString()) // use indexes and convert them to string
    })
  ).data;
  ...
  for (const [index, epochData] of validatorsDataPerEpochMap) {
    ...
    const totalRewards = total_rewards.find((reward) => parseInt(reward.validator_index) === index);
    if (!totalRewards) {
