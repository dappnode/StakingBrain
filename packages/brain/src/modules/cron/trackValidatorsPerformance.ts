...
export async function trackValidatorsPerformance(
  ...
  let previousEpochFinalized: number | null = null;
  ...
  while (true) {
    try {
      ...
      // compare if the finalized epoch has changed
      if (previousEpochFinalized && previousEpochFinalized !== epochFinalized) {
        // continue to the next finalized epoch
        previousEpochFinalized = epochFinalized;
      }
      ...
      // if success, store current epoch and sleep until the next epoch
      previousEpochFinalized = epochFinalized;
      const currentUnixTime = Math.floor(Date.now() / 1000);
      const secondsToNextEpoch = getSecondsToNextEpoch({
        currentUnixTime,
        minGenesisTime
      });
      logger.info(`Sleeping for ${secondsToNextEpoch} seconds until the next epoch`);
      await new Promise((resolve) => setTimeout(resolve, secondsToNextEpoch * 1000));
    } catch (error) {
...
function getSecondsToNextEpoch({
  ...
  const stlotsSinceGenesis = timeDifference / 12; // Slots since genesis
  ...
  const nextEpochStartTime = nextEpochStartSlot * 12 + minGenesisTime; // Time at the start of the next epoch in seconds
