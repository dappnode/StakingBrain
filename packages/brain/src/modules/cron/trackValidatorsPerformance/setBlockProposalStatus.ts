...
export async function setBlockProposalStatus({
  ...
  for (const duty of blockProposalsResponse.data) {
    ...
    const validatorIndexInt = parseInt(validator_index);

    // enter loop if one of our monitored validators had to propose in this slot
    const validatorEpochData = validatorsDataPerEpochMap.get(validatorIndexInt);
    if (validatorEpochData) {
      try {
        // Get the proposer index from the block header for the slot
        const proposerIndex = parseInt(
          (await beaconchainApi.getBlockHeader({ blockId: slot })).data.header.message.proposer_index
        );
        // If the proposer index in the block header matches the validator index, the block was proposed correctly.
        validatorsDataPerEpochMap.set(validatorIndexInt, {
          ...validatorEpochData,
          block: {
            status: proposerIndex === validatorIndexInt ? BlockProposalStatus.Proposed : BlockProposalStatus.Error
          }
        });
      } catch (error) {
        if (error.status === 404) {
